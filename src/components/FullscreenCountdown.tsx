...
const FullscreenCountdown: React.FC<FullscreenCountdownProps> = ({ 
  ...
  const FloatingElements = () => (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {[...Array(20)].map((_, i) => (
        ...
        >
          {i % 3 === 0 ? (
            <Heart className="w-4 h-4 sm:w-6 sm:h-6 text-pink-400" fill="currentColor" />
          ) : i % 3 === 1 ? (
  ...
  if (isComplete) {
    return (
      <motion.div
        className="fixed inset-0 bg-gradient-to-br from-pink-500 via-purple-500 to-indigo-600 z-50 overflow-y-auto"
        initial={{ opacity: 1 }}
        exit={{ opacity: 0, scale: 0.8 }}
        transition={{ duration: 1 }}
      >
        <div className="min-h-full flex items-center justify-center p-4">
          ...
          >
            ...
            >
              <Gift className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 mx-auto mb-4 sm:mb-8 text-yellow-300" />
            </motion.div>
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-bold mb-2 sm:mb-4">
              🎉 IT'S TIME! 🎉
            </h1>
  ...
  return (
    <motion.div
      className="fixed inset-0 bg-gradient-to-br from-pink-100 via-purple-100 to-indigo-200 z-50 overflow-y-auto"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 1 }}
    >
      <div className="min-h-full flex items-center justify-center py-4 px-4">
        ...
        <div className="text-center w-full max-w-6xl">
          <motion.div
            ...
            transition={{ delay: 0.5, duration: 0.8 }}
          >
            <Heart className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 mx-auto text-red-500 mb-4 sm:mb-6 md:mb-8" fill="currentColor" />
          </motion.div>

          <motion.h1
            className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-serif font-bold text-purple-800 mb-2 sm:mb-4"
            initial={{ y: 30, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.7, duration: 0.8 }}
          >
          ...
          <motion.p
            className="text-lg sm:text-xl md:text-2xl text-purple-600 mb-6 sm:mb-8 md:mb-12 max-w-2xl mx-auto px-4"
            initial={{ y: 30, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.9, duration: 0.8 }}
          >
          ...
          <motion.div
            className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 max-w-4xl mx-auto mb-6 sm:mb-8 md:mb-12"
            initial={{ y: 30, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 1.1, duration: 0.8 }}
          >
            {[
              ...
              <motion.div
                key={item.label}
                className="bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-3 sm:p-4 md:p-6 shadow-xl border border-pink-200"
                whileHover={{ scale: 1.05 }}
                initial={{ scale: 0.8, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                transition={{ delay: 1.3 + index * 0.1, duration: 0.5 }}
              >
                <motion.div
                  className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-purple-800 mb-1 sm:mb-2"
                  key={item.value} // This will trigger re-animation on value change
                  initial={{ scale: 1.2 }}
                  animate={{ scale: 1 }}
                  transition={{ duration: 0.3 }}
                ...
                <div className="text-sm sm:text-base md:text-lg text-purple-600 font-medium">
                  {item.label}
                </div>
              </motion.div>
            ))}
          ...
          <motion.button
            className="px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-full font-semibold text-base sm:text-lg shadow-lg hover:shadow-xl transition-all duration-300"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            initial={{ y: 30, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
